...
def draw_objs(image: Image,
    ...
    # 为每个房子分配一个唯一的颜色
    unique_colors = [tuple(random.randint(0, 255) for _ in range(3)) for _ in range(len(boxes))]

    if draw_boxes_on_image:
        # Draw all boxes onto image.
        draw = ImageDraw.Draw(image)
        for box, cls, score, color in zip(boxes, classes, scores, unique_colors):
            left, top, right, bottom = box
            # 绘制目标边界框
            draw.line([(left, top), (left, bottom), (right, bottom),
                       (right, top), (left, top)], width=line_thickness, fill=color)
    ...
    if draw_masks_on_image and (masks is not None):
        # Draw all mask onto image.
        image = draw_masks(image, masks, unique_colors, mask_thresh)
